...
def select_model(args, device):
    ...
    if model_id == -1:
        ...
        #model = RFDN(in_nc=3, out_nc=3, nc=64, nb=8, upscale=4)
        model = RFDN()
    ...
    # print(model)
    model.eval()
    x = torch.randn(1, 3, 744, 1296, requires_grad=False); # you have to get this from a sample run
    #x = torch.zeros(1, 3, 744, 1296)
    # Export the model
    torch.onnx.export(model,               # model being run
                x,                         # model input (or a tuple for multiple inputs)
                "team1.onnx",   # where to save the model (can be a file or file-like object)
